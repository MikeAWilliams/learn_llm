...
class GPT(nn.Module):
    ...
    This class reproduces the GPT-2 model structure and can load pretrained weights
    from HuggingFace or be trained from scratch.
    """
    ...
    def configure_optimizers(
        self, weight_decay, learning_rate, device, master_process=True
    ):
        """
        Configure AdamW optimizer with weight decay.

        Separates parameters into two groups:
        - Parameters with dimension >= 2 (weights) get weight decay
        - Parameters with dimension < 2 (biases, layer norms) don't get weight decay
